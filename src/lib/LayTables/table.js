...
; !function () {
    ...
    var objPage = {
        theme: '#1E9FFF',
        layout: ['first', 'prev', 'page', 'next', 'last', 'limit', 'skip', 'count'],
        first: '首页',
        last: '尾页',
        prev: '上一页',
        next: '下一页',
        ...
        , objResponse = {
            ...
            dataName: 'RowData'       //数据列表的字段名称，默认：data
        }
        , laytpl = JointLayTpl
        , laypage = JointLayPage
        //外部接口
        , table = {
            config: {
                checkName: 'FChecked' //是否选中状态的字段名
                , radioName: 'LAY_RadioCHECKED' //是否选中状态的字段名(张学山 2017-11-18 加表格单选控制)
                , indexName: 'LAY_TABLE_INDEX' //下标索引名
            } //全局配置项
            , cache: {} //数据缓存
            , index: window.JointLayTable ? (window.JointLayTable + 10000) : 0
            ...
            , on: function (events, callback) {
                return layonevent.call(this, MOD_NAME, events, callback);
            }
            //表格自定义属性扩展 2018-06-14 张学山 改 BEGIN
            , Initialize: function (id) {
                //Do Nothings
            }
            , GetWhereSQL: function (id, ctrl, _ffuncid) {
                var _keyword = "";
                if (!CheckIsNullOrEmpty(ctrl)) {
                    _keyword = $("#" + ctrl).val();
                }

                var fn = id + "_FilterSQL";
                var fn2 = null;
                try { fn2 = eval(fn) } catch (ex) { fn2 = fn; }
                if (CheckIsJsFunction(fn2)) {
                    return fn2(_keyword, _ffuncid);
                }
                else {
                    return { FFuncID: _ffuncid, KeyWords: _keyword };
                }
            }
            , RenderTable: function (id) {
                var obj = eval(id + "_JSDS");
                var objWheresql = JointLayTable.GetWhereSQL(id, obj.KeyWordsID, obj.QueryFFuncID);
                ...
                var tableObj = {
                    ...
                    cols: obj.objRowData,
                        ...
                        eval(id + "_DataSource = res;");
                        JointLayTable.InitEvent(id);
                    ...
                    page: (obj.IsShowPager == "false" ? "false" : objPage),
                    limit: obj.PageSize,
                    limits: obj.PageModule,
                    loading: true,
                    //,skin: 'line'     //行边框风格
                    even: true,         //开启隔行背景
                    size: obj.SkinSize, //小尺寸的表格
                    //, initSort: {
                ...
                if (obj.IsShowPager == "false") {
                    tableObj.page = false;
                }
                else {
                    tableObj.page = objPage;
                }

                if (obj.Width != undefined && obj.Width > 0) {
                    tableObj.width = obj.Width;
                }
                else {
                    tableObj.width = "100%";
                }
                if (obj.Height != undefined) {
                    tableObj.height = obj.Height;
                }
            ...
            , InitEvent: function (id) {
                var obj = eval(id + "_JSDS");
                //行选择事件
                $('.layctrl-' + id + ' tr').click(function () {
                    if (obj.SelectMode == 'Single') {
                        if (!$(this).hasClass('selected')) {
                            $('.layctrl-' + id + ' tr.selected').removeClass('selected');
                            $('.layctrl-' + id + ' td .layui-form-radio').removeClass('layui-form-radioed');
                            $('.layctrl-' + id + ' td .layui-form-radio i').removeClass('layui-anim-scaleSpring').addClass("layui-icon-unradio");

                            $(this).addClass('selected');
                            $(this).find("div.layui-form-radio").trigger("click");
                        }
                    }

                    //扩展事件
                    var fn = obj.ClientRowClickEvent;
                ...
                //双击事件
                $('.layctrl-' + id + ' tr').dblclick(function () {
                    if (obj.SelectMode == 'Single') {
                        if (!$(this).hasClass('selected')) {
                            $('.layctrl-' + id + ' tr.selected').removeClass('selected');
                            $('.layctrl-' + id + ' td .layui-form-radio').removeClass('layui-form-radioed');
                            $('.layctrl-' + id + ' td .layui-form-radio i').removeClass('layui-anim-scaleSpring');
                ...
                //默认选中第一行
                if (obj.AutoSelectFirstRow == 'true') {
                    if (obj.SelectMode == 'Single') {
                        $(".layctrl-" + id + " tr:first .layui-form-radio").trigger("click");
                    }
                };
            }
            , GetSelectRowData: function (id) {
                //获取选中行的值
                var obj = eval(id + "_JSDS");
                var data = eval(id + "_DataSource");
                if (obj.SelectMode == 'Single') {
                    if ($('.layctrl-' + id + ' tr.selected').length > 0) {
                        return data.RowData[$('.layctrl-' + id + ' tr.selected').attr('data-index')];
                    }
                }
                else {
                    var ary = new Array();
                    $(".layctrl-" + id + " tr:has(div[lay-value=1])").each(function(){
                        ary.push(data.RowData[$(this).attr('data-index')]);
                    });

                    return ary;
                }
            }
            , GetSelectRowField: function (id, name) {
                //获取选择行某字段的值
                var obj = eval(id + "_JSDS");
                var data = eval(id + "_DataSource");
                if (obj.SelectMode == 'Single') {
                    if ($('.layctrl-' + id + ' tr.selected').length > 0) {
                        return data.RowData[$('.layctrl-' + id + ' tr.selected:first').attr('data-index')][name];
                    }
                    else {
                        return null;
                    }
                    ...
                    var v = "";
                    var ary = JointLayTable.GetSelectRowData(id);
                    if (ary.length > 0) {
                        for (var c in ary) {
                            v += ',' + ary[c][name];
                        }
            ...
            , IsSelectARow: function (id) {
                var obj = eval(id + "_JSDS");
                //返回是否选择行数据
                if (obj.SelectMode == 'Single') {
                    return $('.layctrl-' + id + ' tr.selected').length;
                }
                else {
                    return $(".layctrl-" + id + " tr:has(div[lay-value=1])").length;
                }
            }
            , GetFirstRow: function (id) {
                var obj = eval(id + "_JSDS");
                var data = eval(id + "_DataSource");

                return data.RowData[0];
            }
            //表格自定义属性扩展 2018-06-14 张学山 改 END
        }
        ...
        , TPL_HEADER = function (options) {
            ...
            return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
                ...
                , '<thead>'
                , '{{# layeach(d.data.cols, function(i1, item1){ }}'
                , '<tr>'
                , '{{# layeach(item1, function(i2, item2){ }}'
                ...
                , '<div name="layTableCheckbox" layTableAllChoose="true" class="layui-unselect layui-form-checkbox {{# if(item2[d.data.checkName]){ }}layui-form-checked{{# }; }}" onclick="fnSwitch2State(this);" lay-skin="primary" lay-value="0"><span></span><i class="layui-icon layui-icon-ok"></i></div>' //2018-06-14 张学山 改
                , '{{# } else { }}'
        ...
        //tbody区域模板
        , TPL_BODY = ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table layctrl-{{d.data.id}}" ' //(张学山 2017-11-18 加表格单选控制)
            , '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
            , '<tbody></tbody>'
            , '</table>'].join('')
    ...
    Class.prototype.setInit = function (type) {
        ...
        //初始化列参数
        layeach(options.cols, function (i1, item1) {
            layeach(item1, function (i2, item2) {
                ...
                if (item2.colGroup || item2.colspan > 1) {
                    var childIndex = 0;
                    layeach(options.cols[i1 + 1], function (i22, item22) {
    ...
    Class.prototype.renderToolbar = function () {
        ...
        if (typeof options.defaultToolbar === 'object') {
            layeach(options.defaultToolbar, function (i, item) {
    ...
    Class.prototype.pullData = function (curr, loadIndex) {
        ...
        if (options.url) { //Ajax请求  张学山 2018-06-14 改
            var params = {};
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        var that = this
            ...
            , render = function () { //后续性能提升的重点
                ...
                layeach(data, function (i1, item1) {
                        ...
                        var td = ['<td data-field="' + field + '" data-key="' + key + '" ' + function () {
                            ...
                            , '<div class="layui-table-cell laytable-cell-' + function () { //返回对应的CSS类标识
                                ...
                                if (item3.type === 'checkbox') {
                                    //2018-06-14 张学山 改  根据FChecked字段，控制表格多选时是否自动选中
                                    var checked = function () {
                                        var checkName = table.config.checkName;
                                        //如果是全选
                                        if (item3[checkName]) {
                                            item1[checkName] = item3[checkName];
                                            return item3[checkName] == "1" ? ' layui-form-checked' : '';
                                        }
                                        return tplData[checkName] == "1" ? ' layui-form-checked' : '';
                                    }();

                                    return '<div name="layTableCheckbox" class="layui-unselect layui-form-checkbox' + checked + '" onclick="fnSwitch2State(this);" lay-skin="primary" lay-value="' + (checked == "" ? 0 : 1) + '"><span></span><i class="layui-icon layui-icon-ok"></i></div>';
                                } else if (item3.type === 'numbers') { //渲染序号
                                    return numbers;
                                }
                                //添加单选框的渲染(张学山 2017-11-18 加表格单选控制 BEGIN)
                                if (item3.type === 'radio') {
                                    var checked = function () {
                                        var radioName = table.config.radioName;
                                        if (item3[radioName]) {
                                            item1[radioName] = item3[radioName];
                                            return item3[radioName] ? ' layui-form-radioed' : '';
                                        }
                                        return item1[radioName] ? ' layui-form-radioed' : '';
                                    }();

                                    return '<div class="layui-unselect layui-form-radio' + checked + '" onclick="fnChooseOneByGroup(this);" name="layTableRadio"><i class="layui-anim layui-icon ' + (checked == '' ? 'layui-icon-unradio' : 'layui-anim-scaleSpring layui-icon-radio') + '"></i>';
                                }
                                //添加单选框的渲染(张学山 2017-11-18 加表格单选控制 END)
    ...
    Class.prototype.renderForm = function (type) {
        //2018-06-14 张学山 注释掉以下代码
        //form.render(type, 'LAY-table-' + this.index);
        //form.render("radio", 'LAY-table-' + this.index);//渲染表单中radio(张学山 2017-11-18 加表格单选控制)
    };
    ...
    Class.prototype.sort = function (th, type, pull, formEvent) {
        ...
        if (type === 'asc') { //升序
            thisData = laysort(data, field);
        } else if (type === 'desc') { //降序
            thisData = laysort(data, field, true);
        } else { //清除排序
            thisData = laysort(data, table.config.indexName);
            delete that.sortKey;
        }
        ...
        if (formEvent) {
            layonevent.call(th, MOD_NAME, 'sort(' + filter + ')', {
                field: field
                , type: type
            });
        }
    ...
    Class.prototype.events = function () {
        ...
        that.layTool.on('click', '*[lay-event]', function (e) {
            ...
            layonevent.call(this, MOD_NAME, 'toolbar(' + filter + ')', $.extend({
                event: events
                , config: options
            }, {}));
        });
        ...
        var commonMember = function (sets) {
            ...
            return $.extend({
                ...
                , update: function (fields) { //修改行数据
                    fields = fields || {};
                    layeach(fields, function (key, value) {
                        if (key in data) {
        ...
        that.elem.on('click', 'input[name="layTableCheckbox"]+', function () { //替代元素的 click 事件
            ...
            layonevent.call(this, MOD_NAME, 'checkbox(' + filter + ')', commonMember.call(this, {
                checked: checked
                , type: isAll ? 'all' : 'one'
            }));
        });

        //单选框选择(张学山 2017-11-18 加表格单选控制 BEGIN)
        that.elem.on('click', 'input[name="layTableRadio"]+', function () {
            ...
            layonevent.call(this, MOD_NAME, 'radio(' + filter + ')', {
                checked: checked
                , data: table.cache[that.key][index]
            });
        });
        //单选框选择(张学山 2017-11-18 加表格单选控制END)
        ...
        that.layBody.on('change', '.' + ELEM_EDIT, function () {
            ...
            layonevent.call(this, MOD_NAME, 'edit(' + filter + ')', commonMember.call(this, {
                value: value
                , field: field
            }));
            ...
            othis.siblings(ELEM_CELL).html(function (value) {
                return templet ? function () {
                    return typeof templet === 'function'
                        ? templet(data) : laytpl($(templet).html() || this.value).render(data)
                }() : value;
            }(this.value));
        ...
        that.layBody.on('click', '*[lay-event]', function () {
            var othis = $(this)
                , index = othis.parents('tr').eq(0).data('index');
            layonevent.call(this, MOD_NAME, 'tool(' + filter + ')', commonMember.call(this, {
    ...
    table.init = function (filter, settings) {
        ...
        elemTable.each(function () {
            ...
            othis.find('tbody>tr').each(function (i1) {
                ...
                //如果未定义字段名
                layeach(cols, function (i3, item3) {
    ...
    table.eachCols = function (id, callback, cols) {
        ...
        //重新整理表头结构
        layeach(cols, function (i1, item1) {
            layeach(item1, function (i2, item2) {
                ...
                if (item2.colGroup) {
                    ...
                    layeach(cols[i1 + 1], function (i22, item22) {
        ...
        var eachArrs = function (obj) {
            layeach(obj || arrs, function (i, item) {
        ...
        alink.href = 'data:' + textType + ';charset=utf-8,\ufeff' + encodeURIComponent(function () {
            ...
            layeach(data, function (i1, item1) {
                ...
                if (typeof id === 'object') { //ID直接为表头数据
                    layeach(id, function (i, item) {
                        i1 == 0 && dataTitle.push(item || '');
                    });
                    layeach(table.clearCacheKey(item1), function (i2, item2) {
                        vals.push(item2);
                    });
                } else {
    ...
    table.clearCacheKey = function (data) {
        ...
        delete data[table.config.radioName]; //张学山 2018-06-13 加表格单选处理
    ...
    (
        (typeof define === 'function' && define.amd) ? define(function () { //requirejs加载
            ...
            if(CheckIsNullOrEmpty(window.JointLayTable)){
                window.JointLayTable = table
            }
            else{
                window.JointLayTable = $.extend(window.JointLayTable, table);
            }
        }()
    );
}();
